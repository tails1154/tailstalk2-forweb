...
interface TailsTalkPushNotification {
  title?: string;
  author?: string;
  body: string;
  icon?: string;
...
self.addEventListener("push", (event) => {
  ...
  const notification: TailsTalkPushNotification = JSON.parse(payload);

  if (!notification.title) {
    if (notification.channel) {
      if (notification.channel.channel_type === "DirectMessage") {
        notification.title = notification.author || "TailsTalk 2";
      } else {
        notification.title = `${notification.author} in ${notification.channel.name}`;
      }
    } else {
      notification.title = "TailsTalk 2";
    }
  }
  ...
  event.waitUntil(
    self.registration.showNotification(notification.title || "TailsTalk 2", {
